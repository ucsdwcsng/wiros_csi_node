...
Additionally, we have made scripts available to convert rosbags containing CSI info to .npz files for 
convenient post-processing [here](https://github.com/ucsdwcsng/ros_bearing_sensor).
This repo also contains functionality such as processing the CSI data in real time to give real-time angle of arrival, angle of departure, and automatic calculation of calibration values. 
...
### Automatic
If you are deploying the node on a mobile platform it may be useful to programatically switch channels to gather information about a large number of devices in the environment. We have provided an additional node, `ap_scanner`, which uses your wifi card to intermittently scan for APs and publishes a list sorted by signal strength. Since we expect nearby devices to be associated with the nearest AP and thus on its channel, this is a decent heuristic for deciding which channel to listen to. The node can be run as such:
